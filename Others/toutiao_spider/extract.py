...
c=db.cursor()
c.execute('select id, raw, tag from news where raw like " {%" and tag <> "forum_post" and tag in (select b from (select count(*) as a, tag as b from news where raw like " {%" group by tag) where a > 100)')
rs=c.fetchall()
for r in rs:
